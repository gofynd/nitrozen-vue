  ...
  methods: {
    autocomplete: function(event) {
      ...
      for (index = 0; index < this.dataset.length; index++) {
        ...
        if (
          this.dataset[index].substr(0, inputValue.length).toUpperCase() ==
          ...
          autocompleteItem.innerHTML =
            "<span>" +
            this.dataset[index].substr(0, inputValue.length) +
            "</span>";
          autocompleteItem.innerHTML += this.dataset[index].substr(
            inputValue.length
          );
          /*insert a input field that will hold the current array item's value:*/
          autocompleteItem.innerHTML +=
            "<input type='hidden' value='" + this.dataset[index] + "'>";
...
.nitrozen-autocomplete-items {
  ...
  margin-bottom: 20px;
  box-shadow: 0 2px 4px -1px @BoxShadow, 0 4px 5px 0 @BoxShadow,
    0 1px 10px 0 @BoxShadow;
}
